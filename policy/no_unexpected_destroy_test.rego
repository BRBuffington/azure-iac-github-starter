...
test_noop_allowed if {
	count(deny) == 0 with input as {"resource_changes": [
		{"address": "azurerm_resource_group.this", "change": {"actions": ["no-op"]}},
	]}
	with data.params as {"allow_recreate": false}
}
...
test_delete_denied if {
	count(deny) > 0 with input as {"resource_changes": [
		{"address": "azurerm_resource_group.this", "change": {"actions": ["delete"]}},
	]}
	with data.params as {"allow_recreate": false}
}
...
test_replace_denied if {
	count(deny) > 0 with input as {"resource_changes": [
		{"address": "azurerm_role_assignment.x", "change": {"actions": ["delete", "create"]}},
	]}
	with data.params as {"allow_recreate": false}
}

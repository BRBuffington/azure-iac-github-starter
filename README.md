...
| `examples/foundry-agent-teams/` | Three independent Microsoft Foundry Prompt Agent roots: Terraform public and Standard/BYO private patterns, plus a parameterized Bicep Standard private pattern with delegated PowerShell only for REST-only agent publication. All stage Foundry first, then Bot Service after agent identity creation. |
